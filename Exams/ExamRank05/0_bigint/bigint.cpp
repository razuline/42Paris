...
/*   Updated: 2026/05/10 17:52:00 by erazumov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bigint::operator<(const bigint &other) const
{
	...
	for (int i = _digits.size() - 1; i >= 0; --i) // Start at the back!
	{
		if (_digits[i] != other._digits[i])       // If we find a difference...
			return _digits[i] < other._digits[i]; // That diff decides who is smaller
	}
	return false;
}
...
bigint::operator>=(const bigint &other) const
{
	return !(*this < other); // If it's NOT less, it MUST be greater or equal
}
